...
// Test user credentials for local environment
...
// Main test function
async function testLocalLoginRedirect() {
  console.log('Starting local login redirect test...');
  console.log(`Using test user: ${LOCAL_TEST_USER.email}`);
  ...
  try {
    ...
    // Test scenario: Login with local user and redirect to dashboard
    console.log('\n--- Test Scenario: Local user login with redirect to dashboard ---');
    ...
    await emailInput.type(LOCAL_TEST_USER.email);
    await passwordInput.type(LOCAL_TEST_USER.password);
  ...
  console.log('Local login redirect test completed');
}

// Run the test
testLocalLoginRedirect().catch(console.error); 